...
class FileProcessing
{
    ...
    public function saveParticipantes($participantes, $curso)
{
    ...
    foreach ($participantes as $participante) {
        ...
        if (!$existingParticipante) {
            ...
            $newParticipante->cursos()->attach($cursoModel, ['rol' => $participante['rol']]);
        } else {
        ...
        // Guarda el nuevo rol en la tabla de relación (cursoparticipante)
            $existingParticipante->cursos()->syncWithoutDetaching([$cursoModel->id => ['rol' => $participante['rol']]]);
        
            $existingParticipante->save();
        }
    }

    $cursoModel->update(['lista_cargada' => true]);
